...
"""
     ...
     glBegin(GL_TRIANGLES) определяет треугольники, которые будут отображаться.
     Далее устанавливается цвет с помощью glColor4f, где первые три значения задают  цвет, последнее устанавливает
     прозрачность на половину (50%).
     Для каждого индекса вершины в грани вычисляются соответствующие координаты вершин из списка tetrahedron_vertices.
